...
fn e2e_self_scan_panic_attack_source() {
    ...
    let report =
        assail::analyze(&src_path).expect("self-scan of panic-attack source should succeed");
    ...
    assert!(
        report.statistics.total_lines > 1000,
...
fn e2e_scan_vulnerable_examples() {
    ...
    let report =
        assail::analyze(&examples_dir).expect("scanning examples directory should succeed");
    ...
    let has_unwrap = report.weak_points.iter().any(|wp| {
        wp.category == WeakPointCategory::UnsafeDeserialization
...
fn e2e_report_json_serialization() {
    ...
    let json = serde_json::to_string(&report).expect("report must be JSON-serializable");

    assert!(!json.is_empty());
    assert!(
        json.contains("language") || json.contains("Rust"),
...
fn e2e_report_yaml_serialization() {
    ...
    let yaml = serde_yaml::to_string(&report).expect("report must be YAML-serializable");
...
fn e2e_binary_files_skipped() {
    ...
    fs::write(&binary_path, b"\x7FELF\x02\x01\x01").expect("binary file creation should succeed");

    // Create a Rust source file
    let source_path = temp_dir.path().join("main.rs");
    fs::write(&source_path, "fn main() {}").expect("source file creation should succeed");
...
fn e2e_permission_denied_handling() {
    ...
    fs::write(&unreadable_file, "fn main() {}").expect("file creation should succeed");
...
fn e2e_deterministic_analysis() {
    ...
    let report1 = assail::analyze(&example).expect("first analysis should succeed");
    let report2 = assail::analyze(&example).expect("second analysis should succeed");
    ...
    assert_eq!(
        report1.weak_points.len(),
        report2.weak_points.len(),
        "determinism: same weak points count"
    );

    // Same statistics
    assert_eq!(
        report1.statistics.total_lines,
        report2.statistics.total_lines
    );
}
...
fn e2e_directory_vs_file_consistency() {
    ...
    let file_report = assail::analyze(&example).expect("file analysis should succeed");
    let dir_report = assail::analyze(&examples_dir).expect("directory analysis should succeed");

    // Directory report should contain findings from the file
    assert!(
        dir_report.weak_points.len() >= file_report.weak_points.len(),
