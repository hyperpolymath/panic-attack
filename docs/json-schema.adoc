...
* *v0.2.0*: Added `+file_statistics+` field to AssailReport, all
locations guaranteed non-null
* *v1.0.0*: Schema stabilized, backwards-compatible changes only from
here
* *Future*: New fields may be added, but existing fields will not change
type or be removed
...
Starting with v1.0.0: - *MAJOR version*: Breaking changes to schema
(removed fields, changed types) - *MINOR version*: Backwards-compatible
additions (new fields, new enum values) - *PATCH version*: No schema
changes
