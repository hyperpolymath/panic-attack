...
Attack profiles let you pass custom arguments to target programs during
assaults. Profiles are JSON or YAML and can supply common arguments,
axis-specific arguments, and probe mode.

Ambush runs also accept profiles. The arguments are forwarded to the
target program while the ambient stressors run in parallel.
...
Axis keys: `+cpu+`, `+memory+`, `+disk+`, `+network+`, `+concurrency+`,
`+time+`.
