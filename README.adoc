...
`panic-attack` is a Clap-powered CLI. Each subcommand inherits a shared set of knobs so the UX is consistent whether developers run `assail`, `attack`, `assault`, `assay`, `aggregate`, `temporal`, `panll`, `groove`, or another entry point.
